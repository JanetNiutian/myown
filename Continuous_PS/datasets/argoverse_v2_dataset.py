...
#try:
sys.path.append('/home/niutian/.vscode-server/QCNet-main/av2-api-main/src')
...
class ArgoverseV2Dataset(Dataset):
    ...
    def process(self) -> None:
        for raw_file_name in tqdm(self.raw_file_names):
            ...
            with open(os.path.join(self.processed_dir, f'{raw_file_name}.pkl'), 'wb') as handle:
                pickle.dump(data, handle, protocol=pickle.HIGHEST_PROTOCOL) #序列化data保存在文件handle中，序列模式为pickle.HIGHEST_PROTOCOL
    ...
    def get_agent_features(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for track_id, track_df in df.groupby('track_id'):
            ...
            predict_mask[agent_idx, :self.num_historical_steps] = False #要预测的是true，过去的是false
            if not current_valid_mask[agent_idx]:
                predict_mask[agent_idx, self.num_historical_steps:] = False #如果还没预测物体就消失了，则是要预测的未来是false
    ...
    def _download(self) -> None:
        ...
        if ((os.path.isdir(self.raw_dir) and len(self.raw_file_names) == len(self)) or (os.path.isdir(self.processed_dir) and len(self.processed_file_names) == len(self))):
            return
        self._processed_file_names = []
        self.download()
