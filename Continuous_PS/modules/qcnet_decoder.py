...
from torch_geometric.utils import dense_to_sparse #将密集邻接矩阵转换为由边索引和边属性定义的稀疏邻接矩阵？
...
class QCNetDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.mode_emb = nn.Embedding(num_modes, hidden_dim)  # an Embedding module containing 6 tensors of size 64
        ...
        self.t2m_propose_attn_layers = nn.ModuleList(
            [AttentionLayer(hidden_dim=hidden_dim, num_heads=num_heads, head_dim=head_dim, dropout=dropout,
                            bipartite=True, has_pos_emb=True) for _ in range(num_layers)]  #module及其参数自动进入网络中
        )
    ...
    def forward(self,
        ...
        pos_m = data['agent']['position'][:, self.num_historical_steps - 1, :self.input_dim] #[30, 2]
        head_m = data['agent']['heading'][:, self.num_historical_steps - 1] #[30]
        head_vector_m = torch.stack([head_m.cos(), head_m.sin()], dim=-1) #[30, 2]

        x_t = scene_enc['x_a'].reshape(-1, self.hidden_dim)  #[1500,64]
        x_pl = scene_enc['x_pl'][:, self.num_historical_steps - 1].repeat(self.num_modes, 1) #[210, 64]
        x_a = scene_enc['x_a'][:, -1].repeat(self.num_modes, 1) #[180, 64]
        m = self.mode_emb.weight.repeat(scene_enc['x_a'].size(0), 1)  #tensor(6,64)初始化(均值0，方差0.02),6*64重复30*1变为[180, 64] anchor-free mode queries

        mask_src = data['agent']['valid_mask'][:, :self.num_historical_steps].contiguous() #[30, 50]
        mask_src[:, :self.num_historical_steps - self.num_t2m_steps] = False #[30, 20]
        mask_dst = data['agent']['predict_mask'].any(dim=-1, keepdim=True).repeat(1, self.num_modes) #[30, 6]

        pos_t = data['agent']['position'][:, :self.num_historical_steps, :self.input_dim].reshape(-1, self.input_dim) #[1500, 2]
        head_t = data['agent']['position'][:, :self.num_historical_steps].reshape(-1) #[4500]
        edge_index_t2m = bipartite_dense_to_sparse(mask_src.unsqueeze(2) & mask_dst[:, -1:].unsqueeze(1)) #[2, 573]
        rel_pos_t2m = pos_t[edge_index_t2m[0]] - pos_m[edge_index_t2m[1]] #[573, 2]
        rel_head_t2m = wrap_angle(head_t[edge_index_t2m[0]] - head_m[edge_index_t2m[1]]) #[573]
        r_t2m = torch.stack(
            [torch.norm(rel_pos_t2m[:, :2], p=2, dim=-1),
             ...
             rel_head_t2m,
             (edge_index_t2m[0] % self.num_historical_steps) - self.num_historical_steps + 1], dim=-1) #[573, 4]
        r_t2m = self.r_t2m_emb(continuous_inputs=r_t2m, categorical_embs=None) #[573, 64]
        edge_index_t2m = bipartite_dense_to_sparse(mask_src.unsqueeze(2) & mask_dst.unsqueeze(1)) #[2, 3438]
        r_t2m = r_t2m.repeat_interleave(repeats=self.num_modes, dim=0) #[3438, 64]
        pos_pl = data['map_polygon']['position'][:, :self.input_dim] #[35, 2]
        orient_pl = data['map_polygon']['orientation'] #[35]
        edge_index_pl2m = radius(
            ...
            max_num_neighbors=300) #[2, 744]
        edge_index_pl2m = edge_index_pl2m[:, mask_dst[edge_index_pl2m[1], 0]] #[2, 744]
        rel_pos_pl2m = pos_pl[edge_index_pl2m[0]] - pos_m[edge_index_pl2m[1]] #[744, 2]
        rel_orient_pl2m = wrap_angle(orient_pl[edge_index_pl2m[0]] - head_m[edge_index_pl2m[1]]) #[744]
        r_pl2m = torch.stack(
            [torch.norm(rel_pos_pl2m[:, :2], p=2, dim=-1),
             ...
             rel_orient_pl2m], dim=-1) #[744, 3]
        r_pl2m = self.r_pl2m_emb(continuous_inputs=r_pl2m, categorical_embs=None) #[744, 64]
        edge_index_pl2m = torch.cat([edge_index_pl2m + i * edge_index_pl2m.new_tensor(
            [[data['map_polygon']['num_nodes']], [data['agent']['num_nodes']]]) for i in range(self.num_modes)], dim=1) #[2, 4464]
        r_pl2m = r_pl2m.repeat(self.num_modes, 1) #[4464, 64]
        edge_index_a2m = radius_graph(
            x=pos_m[:, :2],
            r=self.a2m_radius,
            batch=data['agent']['batch'] if isinstance(data, Batch) else None,
            loop=False,
            max_num_neighbors=300) #[2, 514]
        edge_index_a2m = edge_index_a2m[:, mask_src[:, -1][edge_index_a2m[0]] & mask_dst[edge_index_a2m[1], 0]] #[2, 458]
        rel_pos_a2m = pos_m[edge_index_a2m[0]] - pos_m[edge_index_a2m[1]] #[458, 2]
        rel_head_a2m = wrap_angle(head_m[edge_index_a2m[0]] - head_m[edge_index_a2m[1]]) #[458]
        r_a2m = torch.stack(
            [torch.norm(rel_pos_a2m[:, :2], p=2, dim=-1),
             ...
             rel_head_a2m], dim=-1) #[458, 3]
        r_a2m = self.r_a2m_emb(continuous_inputs=r_a2m, categorical_embs=None) #[458, 64]
        edge_index_a2m = torch.cat(
            [edge_index_a2m + i * edge_index_a2m.new_tensor([data['agent']['num_nodes']]) for i in
             range(self.num_modes)], dim=1) #[2, 2748]
        r_a2m = r_a2m.repeat(self.num_modes, 1) #[2748, 64]
        edge_index_m2m = dense_to_sparse(mask_dst.unsqueeze(2) & mask_dst.unsqueeze(1))[0] #[2, 792]
        locs_propose_pos: List[Optional[torch.Tensor]] = [None] * self.num_recurrent_steps #list[None, None, None]
        ...
        for t in range(self.num_recurrent_steps): #3
            #Mode2SceneCrossAttn：由3个CrossAttn构成，循环num_layers次
            for i in range(self.num_layers):
                m = m.reshape(-1, self.hidden_dim)  #初始为180*64,表示anchor-free mode queries
                m = self.t2m_propose_attn_layers[i]((x_t, m), r_t2m, edge_index_t2m) #history (trajectory) encodings with input queries 进行CrossAttn。i=0时维度是180*64,m作为查询向量
                m = m.reshape(-1, self.num_modes, self.hidden_dim).transpose(0, 1).reshape(-1, self.hidden_dim) #[180, 64]
                m = self.pl2m_propose_attn_layers[i]((x_pl, m), r_pl2m, edge_index_pl2m) #[180, 64]
                m = self.a2m_propose_attn_layers[i]((x_a, m), r_a2m, edge_index_a2m) #[180, 64]
                m = m.reshape(self.num_modes, -1, self.hidden_dim).transpose(0, 1).reshape(-1, self.hidden_dim) #[180, 64]
            #Mode2ModeSelfAttn
            m = self.m2m_propose_attn_layer(m, None, edge_index_m2m) #[180, 64]
            m = m.reshape(-1, self.num_modes, self.hidden_dim) #[30, 6, 64]
            locs_propose_pos[t] = self.to_loc_propose_pos(m) #[30,6,120] （和num_layer无关，和output_dim有关）
            scales_propose_pos[t] = self.to_scale_propose_pos(m) #[30, 6, 80]
            if self.output_head:
                locs_propose_head[t] = self.to_loc_propose_head(m) #[30, 6, 20]
                concs_propose_head[t] = self.to_conc_propose_head(m) #[30, 6, 20]
        loc_propose_pos = torch.cumsum(
            torch.cat(locs_propose_pos, dim=-1).view(-1, self.num_modes, self.num_future_steps, self.output_dim),
            dim=-2)  #按照最后一维拼接，重构张量维度为（30*output_dim/num_modes,6,60,output_dim），按照倒数第二维，后面的值是前面的累加 #[30, 6, 60, 4]
        ...
        if self.output_head:
            ...
            conc_propose_head = scale_propose_pos.new_zeros((scale_propose_pos.size(0), self.num_modes,
                                                             self.num_future_steps, 1))
            m = self.y_emb(loc_propose_pos.detach().view(-1, self.output_dim)) #切断反向传播，使其不具有梯度，[10800,64]
        m = m.reshape(-1, self.num_future_steps, self.hidden_dim).transpose(0, 1)
        # GRU is used to embed each trajectory anchor，take its final hidden state as the mode query
        m = self.traj_emb(m, self.traj_emb_h0.unsqueeze(1).repeat(1, m.size(1), 1))[1].squeeze(0) #仍为[180,64]，和anchor-free mode queries维度相同
        for i in range(self.num_layers):
        ...
        loc_refine_pos = self.to_loc_refine_pos(m).view(-1, self.num_modes, self.num_future_steps, self.output_dim) #MLP，输出为[30, 6, 60, 4]
        loc_refine_pos = loc_refine_pos + loc_propose_pos.detach() #[30, 6, 60, 4]
        scale_refine_pos = F.elu_(
            self.to_scale_refine_pos(m).view(-1, self.num_modes, self.num_future_steps, self.output_dim),
            alpha=1.0) + 1.0 + 0.1 #[30, 6, 60, 4]
        if self.output_head:
            ...
            conc_refine_head = scale_refine_pos.new_zeros((scale_refine_pos.size(0), self.num_modes,
                                                           self.num_future_steps, 1)) #[30, 6, 60, 1]
        pi = self.to_pi(m).squeeze(-1) #[30,6]
