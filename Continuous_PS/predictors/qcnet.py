...
from torch_geometric.data import Batch #使用Batch对象可以方便地处理批次数据，例如对批次中所有图形数据进行相同的前向传播和反向传播操作，并可以实现批次级别的节点和边特征归一化
...
class QCNet(pl.LightningModule): #继承类pl.LightningModule
    ...
    def forward(self, data: HeteroData):
        ...
        #     pred = self.decoder(data, map_enc, agent_enc)
        #     data_new = process_pred(pred)
        #     data = data_new
        return pred
    #Lightning特有的框架流程输入（self, batch, batch_idx）
    #训练主要是重写def training_step(self, batch, batch_idx)函数，并返回要反向传播的loss
    def training_step(self,
                      data,
                      batch_idx): #data为从 train_dataloader 采样的一个batch的数据，batch_idx即为目前batch的索引
        if isinstance(data, Batch): #判断对象data是否是Batch类型
            data['agent']['av_index'] += data['agent']['ptr'][:-1] #29+=?
        reg_mask = data['agent']['predict_mask'][:, self.num_historical_steps:] #[30,110]变为[30,60]
        cls_mask = data['agent']['predict_mask'][:, -1] #[30]
        pred = self(data)
        if self.output_head:
            traj_propose = torch.cat([pred['loc_propose_pos'][..., :self.output_dim],
                                      pred['loc_propose_head'],
                                      pred['scale_propose_pos'][..., :self.output_dim],
                                      pred['conc_propose_head']], dim=-1)  #按照最后一维进行拼接
        ...
        pi = pred['pi']
        gt = torch.cat([data['agent']['target'][..., :self.output_dim], data['agent']['target'][..., -1:]], dim=-1) #[30, 60, 3]
    ...
    def validation_step(self,
        ...
        data_new ={'scenario_id':data_new['scenario_id'], 'city':data_new['city'],
            'agent':{'num_nodes':data_new['agent']['num_nodes'], 'av_index':data_new['agent']['av_index'], 'valid_mask':data_new['agent']['valid_mask'].cuda(), 'predict_mask':data_new['agent']['predict_mask'].cuda(), 'id':data_new['agent']['id'], 'type':data_new['agent']['type'].cuda(), 'category':data_new['agent']['category'].cuda(), 'position':data_new['agent']['position'].cuda(), 'heading':data_new['agent']['heading'].cuda(), 'velocity':data_new['agent']['velocity'].cuda(), 'target':data_new['agent']['target'].cuda()},
            'map_polygon':{'num_nodes':data_new['map_polygon']['num_nodes'],'position':data_new['map_polygon']['position'].cuda(),'orientation':data_new['map_polygon']['orientation'].cuda(),'height':data_new['map_polygon']['height'].cuda(),'type':data_new['map_polygon']['type'].cuda(),'is_intersection':data_new['map_polygon']['is_intersection'].cuda(),}, 
            'map_point':{'num_nodes':data_new['map_point']['num_nodes'], 'position':data_new['map_point']['position'].cuda(), 'orientation':data_new['map_point']['orientation'].cuda(), 'magnitude':data_new['map_point']['magnitude'].cuda(), 'height':data_new['map_point']['height'].cuda(), 'type':data_new['map_point']['type'].cuda(), 'side':data_new['map_point']['side'].cuda()}, 
            ('map_point', 'to', 'map_polygon'):{'edge_index':data_new[('map_point', 'to', 'map_polygon')]['edge_index'].cuda()}, ('map_polygon', 'to', 'map_polygon'):{'edge_index':data_new[('map_polygon', 'to', 'map_polygon')]['edge_index'].cuda(),'type':data_new[('map_polygon', 'to', 'map_polygon')]['type'].cuda()}}

        # reg_mask = data_new['agent']['predict_mask'][:, self.num_historical_steps:]
        # cls_mask = data_new['agent']['predict_mask'][:, -1]
        pred = self(data_new)
    ...
    def test_step(self,
        ...
        if isinstance(data, Batch):
            data['agent']['av_index'] += data['agent']['ptr'][:-1]
        pred = self(data)  # 表示调用QCNet实例的forward方法来对输入数据data进行预测
        if self.output_head:
            traj_refine = torch.cat([pred['loc_refine_pos'][..., :self.output_dim],
                                     pred['loc_refine_head'],
                                     pred['scale_refine_pos'][..., :self.output_dim],
        ...
        traj_eval = torch.matmul(traj_refine[eval_mask, :, :, :2],
                                 rot_mat.unsqueeze(1)) + origin_eval[:, :2].reshape(-1, 1, 1, 2)   # torch.matmul表示矩阵乘法
        pi_eval = F.softmax(pi[eval_mask], dim=-1)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--output_head', default=True) # action='store_true'
