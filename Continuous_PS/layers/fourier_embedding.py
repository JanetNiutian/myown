...
class FourierEmbedding(nn.Module):
    ...
    def forward(self,
                continuous_inputs: Optional[torch.Tensor] = None,#可选类型，作用几乎和带默认值的参数等价，不同的是使用Optional会告诉你的IDE或者框架：这个参数除了给定的默认值外还可以是None
                categorical_embs: Optional[List[torch.Tensor]] = None) -> torch.Tensor: #期待的类型
