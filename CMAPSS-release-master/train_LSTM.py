...
model_lstm.eval()
lstm_features, rul_pred = model_lstm(x_test)
...
mean_pred_for_each_engine = torch.tensor(mean_pred_for_each_engine)  # 将列表转换为 PyTorch 张量
mean_pred_for_each_engine = mean_pred_for_each_engine.detach().numpy()
...
plt.ylabel('RUL')
plt.legend()
plt.savefig('/data/niutian/code/CMAPSS-release-master/FD003_LSTM.png')
plt.show()
