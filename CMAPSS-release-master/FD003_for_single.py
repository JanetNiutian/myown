...
def GPR_train(train_loader):
    ...
    indices = np.random.choice(total_samples, 5300, replace=False)  #5300
...
def GPR_test(gpr,test_loader,engine_id):
    ...
    # 时间轴（样本）
    # 时间轴（从第35个操作周期开始）
    time = np.arange(95, 95 + len(test_labels))
    # # 可视化预测结果和实际值
    # 设置图片的长宽比，增加高度，减少宽度
    ...
    # plt.legend()
    plt.savefig(f'/home/niutian/原data/code/CMAPSS-release-master/single/FD003_{engine_id}_GPR.pdf')
    # plt.show()
...
if __name__ == '__main__':
    ...
    for i in range(1, 101):
        if i == 20 or i == 99 :
            engine_id = i
            test_egine = get_test_engine(dir_path='/home/niutian/原data/code/CMAPSS-release-master/CMAPSSData/', sub_dataset='FD003', max_rul=125, seq_length=30, smooth_rate=128, engine_id=i)
